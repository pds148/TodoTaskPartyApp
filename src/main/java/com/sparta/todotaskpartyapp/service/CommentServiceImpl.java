...
import com.sparta.todotaskpartyapp.exception.CommentNotFoundException;
...
public class CommentServiceImpl implements CommentService {
    ...
    private Comment getUserComment(Long commentId, User user) {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new CommentNotFoundException("존재하지 않는 댓글 ID 입니다."));

        if (!user.getId().equals(comment.getUser().getId())) {
            throw new CommentNotFoundException("작성자만 수정할 수 있습니다.");
        }
        return comment;
    }
}
